...
module.exports = function engage( options, register ) {
  ...
  function ensure_engage(seneca,token,error,handler) {
    engage_ent.load$({token:token},function(err,engage){
      ...
      create_engage(function(err,engage){
        seneca.log.debug('create', function(){ return token ? ['not-found',token] : ['not-present']})
        if( err ) return error(err);
  ...
  this.add({role:name,cmd:'set'},function(args,cb){
    //console.log('EGET req$.s.et:'+!!args.req$.seneca.engage_token)
  ...
  this.add({role:name,cmd:'wrap'},function(args,done){
    ...
    this.wrap(args.pin,function(args,done){
      ...
      this.util.recurse.call(
        ...
        function(err,args){
          //console.log('EWRAP PARENT cart:'+args.cart)
          parent(args,function(err,out){
            ...
            this.util.recurse.call(
              ...
              keys,
              function(key,next){
                if( _.isObject(out) && _.isUndefined(out[key]) ) {
                  next()
                }
                else {
                  //console.log('EWRAP SET '+key)
                  var value = _.isObject(out) ? out[key] : null
                  this.act({role:name,cmd:'set',key:key,value:value, context:args.context,req$:args.req$,res$:args.res$},function(err,val){
                    if( err ) return done(err);
                    next()
                  })
                }
